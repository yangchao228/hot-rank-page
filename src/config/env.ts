...
const envSchema = z.object({
  ...
  UPSTREAM_BASE_URL: z.string().url().default("https://api-hot.imsyy.top"),
  ...
  RETRY_TIMES: z.coerce.number().int().min(0).max(5).default(2),
  CIRCUIT_FAIL_THRESHOLD: z.coerce.number().int().min(1).default(3),
  CIRCUIT_COOLDOWN_MS: z.coerce.number().int().min(1000).default(30000),
